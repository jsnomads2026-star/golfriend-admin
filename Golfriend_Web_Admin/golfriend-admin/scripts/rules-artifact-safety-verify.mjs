...
// ---- 4. THE EXISTING COMPLETE AUTHORITY RULESET REMAINS BOUND -----------------------
const firebaseConfig = JSON.parse(readFileSync(resolve(ROOT, 'firebase.json'), 'utf8'));
assert.equal(firebaseConfig.firestore?.rules, 'enterprise-authority.firestore.rules',
  'firebase.json must retain the validated complete enterprise authority ruleset');
...
ok('firebase.json retains the validated complete enterprise authority ruleset');

console.log(`\nRules artifact safety PASS: ${checks} checks (${PROTECTED_COLLECTIONS.length} protected collections, partial ruleset rejected, ${HOSTILE.length} hostile candidates rejected, complete authority ruleset retained).`);
