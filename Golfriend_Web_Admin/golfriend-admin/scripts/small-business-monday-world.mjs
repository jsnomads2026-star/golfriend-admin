...
export const verifyWorld=(world=loadWorld())=>{
 ...
 require(world.schema==='golfriend.small-business.monday-world.v2','schema');require(world.productionWrites===false,'productionWrites');require(world.identities.length===3,'identities');require(world.businesses.length===6,'businesses');require(world.scenarios.length===39,'scenarios');
 ...
 const cafe=world.businesses.find(value=>value.businessId==='sb_cafe_siam');require(cafe?.locations.length===2,'two-location cafe');require(world.mobileProducerFacts?.profileVersions?.before===4&&world.mobileProducerFacts?.profileVersions?.after===5,'profile versions');require(world.mobileProducerFacts?.locationVersions?.loc_cafe_bangna===3,'location version');require(world.mobileProducerFacts?.locales?.length===8,'mobile locales');require(world.mobileProducerFacts?.inquiries?.every(value=>value.state!=='transmitted'),'no fake transmitted inquiry');require(world.reportingFacts.promotionGolfriendRevenue===0,'zero promotion revenue');require(world.reportingFacts.externalCommissionIsGolfriendRevenue===false,'external commission separation');require(world.reportingFacts.jhccTransmission===false,'no JHCC transmission');
