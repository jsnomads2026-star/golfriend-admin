...
t("enterprise outbox is App Check protected immutable and not transmitted",()=>{const source=r("functions/src/enterpriseBookingOutboxRuntime.ts");assert.match(source,/transitionEnterpriseCorrelatedBookingV2=onCall\(\{enforceAppCheck:true/);assert.match(source,/transactionBookingAuthority/);assert.match(source,/deliveryState:"awaiting_golfer_consumer"/);assert.doesNotMatch(source,/collection\("play_bookings"\)|send|publish/)});
t("token recovery is V2 only and fail closed until issuer and consumer exist",()=>{const source=r("functions/src/partnerBookingTokenRecoveryRuntime.ts");assert.match(source,/recoverPlayBookingConfirmationV2=onCall\(\{enforceAppCheck:true/);assert.match(source,/BOOKING_TOKEN_RECOVERY_COMMISSIONED=false/);assert.match(source,/play_booking_operation_authorizations_v2/);assert.match(source,/play_booking_confirmation_tokens_v2/);assert.match(source,/state:"revoked"/);assert.match(source,/booking_token_recovery_authority_unavailable/)});
