...
t("legacy booking commands reject and hide V2 correlated bookings",()=>{const schema=/golfriend\.enterprise-correlated-booking\.v2/g;assert.ok((runtime.match(schema)||[]).length>=7);const preview=runtime.slice(runtime.indexOf("previewPlayBookingActionV2"),runtime.indexOf("managePlayBookingV2")),manage=runtime.slice(runtime.indexOf("managePlayBookingV2"),runtime.indexOf("sendPlayBookingMessageV2")),message=runtime.slice(runtime.indexOf("sendPlayBookingMessageV2"),runtime.indexOf("getPlayBookingsPortalV2"));assert.match(preview,schema);assert.ok((manage.match(schema)||[]).length>=3);assert.ok((message.match(schema)||[]).length>=2)});
