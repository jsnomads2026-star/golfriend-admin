...
const domain = read("functions/src/bookingProviderPublicationDomain.ts"),
  runtime = read("functions/src/bookingProviderPublicationRuntime.ts"),
  ui = read("src/components/B2B/BookingProviderPublicationV2.tsx"),
  index = read("functions/src/index.ts"),
  rules = read("partner-onboarding.firestore.rules"),
  app = read("src/App.tsx"),
  small = read("src/components/B2B/SmallBusinessDashboard.tsx"),
  enterprise = read("src/components/B2B/EnterpriseDashboard.tsx");
let count=0;const check=(name,value)=>{assert(value,name);count++;console.log(`PASS ${name}`)};
const blockForLocale=(locale)=>{
  if(locale === "en"){
    const start = ui.indexOf("const EN: Copy = {");
    const end = ui.indexOf("};\n\nconst COPY", start);
    return ui.slice(start, end);
  }
  const key = `\n  ${locale}: {`;
  const start = ui.indexOf(key);
  if(start < 0) return "";
  const end = ui.indexOf("\n  },", start);
  return ui.slice(start, end >= 0 ? end : ui.length);
};
...
for (const locale of ["en", "th", "ko", "ja", "zh", "es", "fr", "de"]) {
  const copyBlock = blockForLocale(locale);
  check(
    `complete ${locale} locale`,
    copyBlock.length > 0 &&
      ["title", "loading", "empty", "error", "retry", "unconfigured", "course", "organization", "prepare", "attempt", "status", "statusUnknown", "boundary", "statusMap", "providerStatusMap"].every(
        (field) => copyBlock.includes(`${field}:`)
      )
  );
  check(
    `${locale} locale has status map values`,
    ["draft", "prepared", "prepared_for_review", "published", "failed", "queued", "sent"].every(
      (value) => copyBlock.includes(`${value}:`)
    )
  );
  check(
    `${locale} locale has provider status map values`,
    ["queued", "sent", "accepted", "rejected", "blocked", "ignored", "unavailable", "done"].every(
      (value) => copyBlock.includes(`${value}:`)
    )
  );
}
check(
  "status rendering localizes item status",
  ui.includes("labelFor(copy.statusMap") && ui.includes("labelFor(copy.providerStatusMap")
);
check("loading state localized", /copy\.loading/.test(ui));
check("empty state localized", /copy\.empty/.test(ui));
check("error state localized", /copy\.error/.test(ui));
check("retry control", /copy\.retry/.test(ui) && /aria-label={copy.retry}/.test(ui));
check("accessible controls", ui.includes("aria-label={copy.prepare}") && ui.includes("aria-label={`${copy.attempt}"));
check("accessible labels", /aria-labelledby/.test(ui) && /role=\"status\"/.test(ui) && /aria-live=\"polite\"/.test(ui));
