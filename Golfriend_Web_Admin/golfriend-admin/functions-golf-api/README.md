...
`syncCoursesFromProvider` is the only exported function. It runs in `asia-southeast1`, requires an active server-owned `admin_users/{uid}` staff or Director role, accepts at most 25 explicit course ids, and uses a Preview before an Apply.
...
```powershell
firebase --config firebase.golf-api.json deploy --only functions:golf-api:syncCoursesFromProvider --project golfriend-v2-production-2ee34
```
...
Retain the existing `courses` client-write denial. The callable returns the caller's preview/apply result; no browser read to the status, preview, or audit collections is required.
