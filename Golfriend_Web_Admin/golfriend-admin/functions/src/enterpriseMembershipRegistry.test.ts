...
//
// Hostile tests for the membership vocabulary. Every case here is something that comes
// back from `collectionGroup('members')` looking exactly like enterprise staff.
// ==========================================
...
// The reason this module exists: a collection-group query matches a NAME, not a
// location. None of these is an enterprise membership; none may influence a grant.
const FOREIGN_PATHS = [
...
// The canonical path IS recognized, so the above is a control, not a blanket reject.
...
assert.equal(evaluate('enterprise_staff/ent-9/members/staff-1', good({ enterpriseUid: 'ent-9' })).verdict, 'cross_enterprise');
assert.equal(evaluate(`enterprise_staff/${CALLER}/members/${STAFF}`, good({ organizationId: 'org-9' })).verdict, 'cross_organization');
...
for (const [label, data] of MALFORMED) {
  assert.equal(evaluate(`enterprise_staff/${CALLER}/members/${STAFF}`, data).verdict, 'malformed', label);
}
// A PRE-REGISTRY record is called out separately: it is not corrupt, it is unevaluable,
// and it must never be read as "unattached".
...
// PAGINATION BEYOND TEN: the blocking record sits at position 24 of 30.
...
// Length-prefixing: no shift of a field boundary can collide. Without it, ('ab','c')
// and ('a','bc') produce the same string.
assert.notEqual(
...
for (const role of ENTERPRISE_STAFF_ROLES) assert.equal(isEnterpriseStaffRole(role), true, role);
// Case variants are REFUSED, never coerced: the old handler silently rewrote any
// unrecognized role to venue_staff, granting something nobody asked for.
for (const bad of ['MANAGER', 'Manager', 'Director', 'superuser', '', null, undefined, 42, {}]) {
  assert.equal(isEnterpriseStaffRole(bad), false, JSON.stringify(bad));
}
for (const bad of ['', 'short', 'a'.repeat(65), 'has space', 'has/slash', null, undefined, 42, {}]) {
  assert.equal(isValidCommandId(bad), false, JSON.stringify(bad));
}
for (const okId of ['cmd-abcdefgh', 'A'.repeat(64), '12345678', 'a_b-c_d1']) {
  assert.equal(isValidCommandId(okId), true, okId);
}
ok(`${REMOVAL_REASONS.length} removal reasons and ${ENTERPRISE_STAFF_ROLES.length} staff roles accepted; case variants and 8 malformed command ids refused`);
