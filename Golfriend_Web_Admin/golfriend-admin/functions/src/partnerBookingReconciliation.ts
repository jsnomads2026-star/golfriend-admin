...
export type BookingReconciliationRequest=Readonly<{actorUid:string;role:"organization_owner"|"organization_admin";organizationId:string;bookingId:string;operationId:string;commandId:string;outcome:BookingReconciliationOutcome;evidence:Readonly<Record<string,unknown>>}>;
...
export function validateBookingReconciliationRequest(raw:Record<string,unknown>):BookingReconciliationRequest{
 const keys=["actorUid","role","organizationId","bookingId","operationId","commandId","outcome","evidence"];if(!exact(raw,keys))throw new Error("RECONCILIATION_FIELDS_INVALID");const outcome=String(raw.outcome)as BookingReconciliationOutcome,role=String(raw.role);if(!ID.test(String(raw.actorUid))||!ID.test(String(raw.organizationId))||!ID.test(String(raw.bookingId))||!ID.test(String(raw.operationId))||!ID.test(String(raw.commandId))||!["organization_owner","organization_admin"].includes(role)||!BOOKING_RECONCILIATION_OUTCOMES.includes(outcome)||!raw.evidence||typeof raw.evidence!=="object"||Array.isArray(raw.evidence))throw new Error("RECONCILIATION_INVALID");
