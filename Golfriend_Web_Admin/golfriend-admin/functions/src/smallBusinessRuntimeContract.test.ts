...
test("V2 cards are exact location projections and navigation remains version separated",()=>{const card=compactSrc.slice(compactSrc.indexOf("functionapprovedCardV2"),compactSrc.indexOf("asyncfunctionactiveBusinessVersion"));for(const key of["businessId","profileVersion","profileDigest","locationId","locationVersion","displayName","category","city:String(l.city)","country:String(l.countryCode)","description","availability","partner","sponsored","disclosure","authoritativeStatus","returnRoute:\"/v2\""])assert.ok(card.includes(key),key);assert.doesNotMatch(card,/\.\.\.card|publicName:|countryCode:|serviceDescription:/);const discovery=compactSrc.slice(compactSrc.indexOf("exportconstdiscoverSmallBusinessesV2"),compactSrc.indexOf("exportconstgetSmallBusinessDetailV2"));assert.match(discovery,/f.city===undefined\|\|z.city===f.city/);assert.match(discovery,/f.countryCode===undefined\|\|z.countryCode===f.countryCode/);const v1=compactSrc.slice(compactSrc.indexOf("exportconstdiscoverSmallBusinessesV1"),compactSrc.indexOf("constMOBILE_SCHEMA_V2"));assert.match(v1,/returnRoute:\"\/lounge\"/);});
test("V2 exact replay precedes mutable authority and returns stored immutable result",()=>{const v2=compactSrc.slice(compactSrc.indexOf("exportconstrecordSmallBusinessEngagementV2"),compactSrc.indexOf("exportconstgetSmallBusinessReportingAdminV1"));for(const token of["returnold.data()?.result","returnh.data()?.result","originalResult:result","issuedAt,result","state:next,result"])assert.ok(v2.includes(token),token);const engagement=v2.slice(0,v2.indexOf("asyncfunctionlistMemberEngagements"));assert.ok(engagement.indexOf("old=awaittx.get(ref)")<engagement.indexOf("activeBusinessVersion(tx"));const prepare=v2.slice(v2.indexOf("exportconstprepareSmallBusinessInquiryV2"),v2.indexOf("asyncfunctionownedInquiry"));assert.ok(prepare.indexOf("h=awaittx.get(history)")<prepare.indexOf("activeBusinessVersion(tx"));assert.ok(prepare.indexOf("h=awaittx.get(history)")<prepare.indexOf("issuedAt=newDate()"));const cancel=v2.slice(v2.indexOf("exportconstcancelSmallBusinessInquiryV2"));assert.ok(cancel.indexOf("h=awaittx.get(history)")<cancel.indexOf("activeBusinessVersion(tx"));});
test("V2 bounded reads detect overflow before filtering or latest-state derivation",()=>{const discovery=compactSrc.slice(compactSrc.indexOf("exportconstdiscoverSmallBusinessesV2"),compactSrc.indexOf("exportconstgetSmallBusinessDetailV2"));assert.match(discovery,/limit\(101\)\.get\(\)/);assert.ok(discovery.indexOf("snap.size>100")<discovery.indexOf("snap.docs.map"));assert.match(discovery,/DISCOVERY_SCOPE_REQUIRES_INDEXED_QUERY/);const saved=compactSrc.slice(compactSrc.indexOf("asyncfunctionlistMemberEngagements"),compactSrc.indexOf("exportconstlistSmallBusinessFavoritesV2"));assert.match(saved,/limit\(201\)\.get\(\)/);assert.ok(saved.indexOf("snap.size>200")<saved.indexOf("for(constdofsnap.docs)"));assert.match(saved,/SAVED_SCOPE_REQUIRES_INDEXED_QUERY/);});
test("V2 detail promotions are an explicit minimum-safe allowlist",()=>{const detail=compactSrc.slice(compactSrc.indexOf("exportconstgetSmallBusinessDetailV2"),compactSrc.indexOf("exportconstrecordSmallBusinessEngagementV2"));for(const key of["promotionId:p.promotionId","locationIds:p.locationIds","contentDigest:p.contentDigest","disclosure:\"sponsored_partner\"","effectiveAt:iso(p.effectiveAt)","expiresAt:iso(p.expiresAt)"])assert.ok(detail.includes(key),key);assert.doesNotMatch(detail,/\.\.\.p|contentReference|audience|policyApproval|contractApproval/);});
test("V2 inquiry history detects bounded overflow before sort or page",()=>{const history=compactSrc.slice(compactSrc.indexOf("exportconstlistSmallBusinessInquiryHistoryV2"),compactSrc.indexOf("exportconstcancelSmallBusinessInquiryV2"));assert.match(history,/limit\(201\)\.get\(\)/);assert.match(history,/INQUIRY_HISTORY_SCOPE_REQUIRES_INDEXED_QUERY/);assert.ok(history.indexOf("snap.size>200")<history.indexOf("snap.docs.sort"));assert.ok(history.indexOf("snap.size>200")<history.indexOf("docs.slice"));});
