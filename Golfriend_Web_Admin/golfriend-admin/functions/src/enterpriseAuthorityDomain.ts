...
export function hierarchyEffective(x:any,now:string){const time=(v:any)=>v?.toMillis instanceof Function?v.toMillis():Date.parse(v);return!!x&&x.status==="active"&&!x.suspendedAt&&!x.revokedAt&&(!x.effectiveAt||time(x.effectiveAt)<=Date.parse(now))&&(!x.expiresAt||time(x.expiresAt)>Date.parse(now))}
export function activeCourseAuthority(m:AuthorityRecord,g:GrantRecord[],hierarchy:Hierarchy,now:string,capability:string){
 ...
 if(!hierarchyEffective(org,now)||!hierarchyEffective(property,now)||!hierarchyEffective(course,now))return false;
