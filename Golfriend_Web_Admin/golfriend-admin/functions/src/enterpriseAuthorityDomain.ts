...
export type BookingAuthorityMode="read"|"mutate";
export type BookingCourseAuthority={membershipId:string;organizationId:string;propertyId:string;courseId:string;role:EnterpriseRole;mode:BookingAuthorityMode;canMutate:boolean;sourceVersion:string};
...
const BOOKING_MUTATION_ROLES=new Set<EnterpriseRole>(["organization_owner","organization_admin","course_manager","booking_staff"]);
const BOOKING_READ_ROLES=new Set<EnterpriseRole>([...BOOKING_MUTATION_ROLES,"analyst_viewer"]);
function bookingHierarchyEffective(value:any,now:string){const instant=(candidate:any)=>candidate?.toMillis instanceof Function?candidate.toMillis():Date.parse(candidate),at=Date.parse(now);return!!value&&value.status==="active"&&!value.suspendedAt&&!value.revokedAt&&(!value.effectiveAt||instant(value.effectiveAt)<=at)&&(!value.expiresAt||instant(value.expiresAt)>at);}
function exactActiveBookingGrant(m:AuthorityRecord,grants:GrantRecord[],organizationId:string,propertyId:string,courseId:string,now:string){const referenced=grants.filter(g=>m.grantIds.includes(g.grantId));if(referenced.length!==m.grantIds.length||referenced.some(g=>g.membershipId!==m.membershipId||g.organizationId!==organizationId))return false;return referenced.some(g=>g.propertyId===propertyId&&g.courseId===courseId&&g.capabilities.includes("booking")&&effective(g.status,g.effectiveAt,g.expiresAt,now));}
function exactActiveHierarchy(hierarchy:Hierarchy,organizationId:string,propertyId:string,courseId:string,now:string){const organization=hierarchy.organizations.find(x=>x.organizationId===organizationId),property=hierarchy.properties.find(x=>x.propertyId===propertyId&&x.organizationId===organizationId),course=hierarchy.courses.find(x=>x.courseId===courseId&&x.propertyId===propertyId&&x.organizationId===organizationId);return bookingHierarchyEffective(organization,now)&&bookingHierarchyEffective(property,now)&&bookingHierarchyEffective(course,now)&&Array.isArray(organization.propertyIds)&&organization.propertyIds.includes(propertyId)&&Array.isArray(property.courseIds)&&property.courseIds.includes(courseId);}
export function resolveBookingCourseAuthority(uid:string,rawMemberships:any[],rawGrants:any[],hierarchy:Hierarchy,target:{organizationId:string;propertyId:string;courseId:string;mode:BookingAuthorityMode},now:string):BookingCourseAuthority{const organizationId=stableId(target?.organizationId,"ORGANIZATION"),propertyId=stableId(target?.propertyId,"PROPERTY"),courseId=stableId(target?.courseId,"COURSE"),mode=target?.mode;if(mode!=="read"&&mode!=="mutate")throw new Error("BOOKING_AUTHORITY_MODE_INVALID");const resolved=resolveAuthority(uid,rawMemberships,rawGrants,hierarchy,now);if(!exactActiveHierarchy(hierarchy,organizationId,propertyId,courseId,now))throw new Error("BOOKING_HIERARCHY_INACTIVE");const matches=resolved.memberships.filter(m=>{if(m.organizationId!==organizationId||!effective(m.status,m.effectiveAt,m.expiresAt,now)||!BOOKING_READ_ROLES.has(m.role))return false;if(mode==="mutate"&&!BOOKING_MUTATION_ROLES.has(m.role))return false;if(m.scope.kind==="course"&&(m.scope.propertyId!==propertyId||m.scope.courseId!==courseId))return false;return exactActiveBookingGrant(m,resolved.grants,organizationId,propertyId,courseId,now);});if(matches.length!==1)throw new Error(matches.length?"BOOKING_AUTHORITY_AMBIGUOUS":"BOOKING_AUTHORITY_DENIED");const membership=matches[0];return Object.freeze({membershipId:membership.membershipId,organizationId,propertyId,courseId,role:membership.role,mode,canMutate:BOOKING_MUTATION_ROLES.has(membership.role),sourceVersion:resolved.sourceVersion});}
