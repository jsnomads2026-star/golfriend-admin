...
await check("cancelled requires exact provider-owned signed evidence", () => {
  const accepted = {...booking, status: "cancellation_accepted"}, acceptedCorrelation = {...correlation, status: "cancellation_accepted"}, unsigned = {schema: "golfriend.enterprise-booking-cancellation-evidence.v2", evidenceId: "evidence_12345678", authority: "commissioned_provider", bookingId: booking.bookingId, correlationId: booking.correlationId, organizationId: booking.organizationId, courseId: booking.courseId, bookingVersion: 1, outcome: "cancelled", issuedAt: "2027-01-01T03:01:00.000Z"}, evidence = {...unsigned, integrityDigest: signEnterpriseBookingCancellationEvidence(unsigned, evidenceSecret)};
