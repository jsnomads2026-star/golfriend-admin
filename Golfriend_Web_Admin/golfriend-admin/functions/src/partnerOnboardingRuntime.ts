...
import {applicationId, canReview, evidenceId, immutableReceipt, PARTNER_SCHEMA, validateDraft, validateEvidence, validateSubmit} from "./partnerOnboardingDomain.js";
...
export const submitPartnerApplicationV2 = onCall({enforceAppCheck: true}, async (request) => {
  ...
  return db.runTransaction(async (tx) => {
    const snapshot = await tx.get(ref);
    if (!snapshot.exists || !validateSubmit(snapshot.data())) throw new HttpsError("failed-precondition", "Complete and save the application first.");
    ...
    tx.update(ref, {status: "submitted", verifiedContact: {emailVerified: true, source: "firebase_auth"}, submittedAt: at(), updatedAt: at()});
...
export const reviewPartnerApplicationV2 = onCall({enforceAppCheck: true}, async (request) => {
  ...
  return db.runTransaction(async (tx) => {
    ...
    tx.update(ref, {status: to, reviewNote: note, reviewedAt: at(), updatedAt: at(), approvedCourseCandidate: to === "approved"});
    ...
    if (to === "approved") tx.set(db.collection("course_growth_candidates").doc(id), {schema: "golfriend.course-growth-candidate.v1", sourceApplicationId: id, organization: snapshot.data()?.organization, course: snapshot.data()?.course, country: snapshot.data()?.country, status: "admin_review_required", publishToApp: false, createdAt: at()}, {merge: false});
    return {success: true, status: to, receipt, courseCandidateCreated: to === "approved", publishedToApp: false};
  });
});
