...
export function verifyBookingConfirmation(token:unknown,record:unknown,expected:BookingConfirmationBinding,nowMs:number,allowUsed=false){
 const value=record as BookingConfirmationRecord;if(typeof token!=="string"||!/^pbc_[A-Za-z0-9_-]{43,}$/.test(token)||!value||value.schema!==BOOKING_CONFIRMATION_SCHEMA||!valid(expected)||!valid(value)||(!allowUsed&&value.used!==false)||(allowUsed&&value.used!==true)||!Number.isSafeInteger(nowMs)||nowMs<value.issuedAtMs||(!allowUsed&&nowMs>=value.expiresAtMs)||!same(value,expected))throw new Error("CONFIRMATION_INVALID");
