...
export const recoverPlayBookingConfirmationV2=onCall({enforceAppCheck:true,secrets:[TOKEN,OUTBOX]},async request=>{
 ...
 return db.runTransaction(async tx=>{
  ...
  if(b.version!==intent.expectedVersion||a.state!=="active")throw new HttpsError("failed-precondition","Booking recovery unavailable.");
  if(prior.exists){
