...
const assertNonFinancial = (input: any) => {
  const blocked = ["price", "amount", "fee", "payment", "wallet", "ledger", "settlement", "refund", "payout", "escrow"];
  if (Object.keys(input || {}).some((key) => blocked.some((field) => key.toLowerCase().includes(field)))) throw new Error("FINANCIAL_FIELD_DENIED");
};
