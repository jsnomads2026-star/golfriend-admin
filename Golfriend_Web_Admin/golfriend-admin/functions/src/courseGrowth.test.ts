...
await check("club-house normalization preserves bounded authority and explicit layout identity", () => {
  ...
  const record=buildClubhouseRecord(clubs[0]);assert.equal(clubs.length,1);assert.equal(clubs[0].phone,"123");assert.equal(clubs[0].providerPropertyId,"property_1");assert.deepEqual(clubs[0].layouts.map(layout=>layout.providerCourseId),["layout_1","layout_2"]);assert.equal((record.bookingCapability as any).provider.status,"unknown","contact and booking URL do not create booking capability");assert.equal((record.bookingCapability as any).golfriendOperational.status,"unverified");assert.doesNotMatch(JSON.stringify(record),/token|secret|authorization/i);
});
