...
const staff = (uid: string): CallerContext =>
  ({ uid, adminDoc: { role: "Manager", status: "Active" }, appCheckVerified: true });
...
check("forged role", () => {
  ...
  assert.equal(authorizeCaller({ uid: "u", adminDoc: { role: "Manager", status: "Suspended" }, appCheckVerified: true })?.code, "not_admin");
  ...
  assert.equal(authorizeCaller({ uid: null, adminDoc: { role: "Manager", status: "Active" }, appCheckVerified: true })?.code, "unauthenticated");
});
...
check("forged reviewer reference", () => {
  for (const surrogate of ["actor-9f2a", "prospect-11", "contact-ab", "rcpt-3", "ref-0"]) {
    ...
    assert.equal(decideAssignment({
      caller: { uid: surrogate, adminDoc: { role: "Manager", status: "Active" }, appCheckVerified: true },
    ...
    assert.equal(move({ uid: surrogate, adminDoc: { role: "Manager", status: "Active" }, appCheckVerified: true }, { assignedReviewerKey: identityKey(surrogate) }).code, "payload_rejected", surrogate);
  }
