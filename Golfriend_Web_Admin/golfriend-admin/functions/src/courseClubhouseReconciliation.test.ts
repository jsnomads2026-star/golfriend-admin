...
assert.match(preview, /parseCataloguePreviewRequest/); assert.match(preview, /cataloguePreviewPage/); assert.match(preview, /catalogueInvalidProviderRows/); assert.match(preview, /MAX_CATALOGUE_PREVIEW_SCAN_ROWS/); assert.match(preview, /STALE_PREVIEW/); assert.doesNotMatch(preview, /executeCourseClubhouseReconciliation/); assert.match(source, /_get\(readTime\)/); assert.match(source, /MAX_RECONCILIATION_WRITES \+ 1/); assert.match(source, /PROVIDER_GROUP_TOO_LARGE/);
