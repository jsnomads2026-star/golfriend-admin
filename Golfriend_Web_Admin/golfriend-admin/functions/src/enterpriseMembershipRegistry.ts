...
//
// The authoritative vocabulary for enterprise staff membership: which document paths are
// membership records at all, which fields a membership may carry, and what a candidate
// document means once it has been read.
//
// WHY THIS EXISTS. The cross-enterprise safety check ran `collectionGroup('members')` and
// treated every returned document as an enterprise membership. A collection-group query
// matches a subcollection NAME, not a location — `members` under any other root, in any
// other domain, in this project or a future one, came back looking exactly like enterprise
// staff. The check could therefore refuse a legitimate grant because of an unrelated
// document, or be fed one deliberately.
//
...
/**
 * The ONLY shape a membership document may have. Anchored at both ends: a path is a
 * membership because of where it is, never because of what it is called.
 */
...
/**
 * Every field a membership document may carry. A surplus field means something wrote to
 * this collection that is not this code, and the safe reading of that is "I do not
 * understand this record", not "the parts I recognize are fine".
 */
export const MEMBERSHIP_FIELDS: readonly string[] = Object.freeze([
  'staffUid', 'email', 'role', 'status', 'enterpriseUid', 'organizationId', 'grantSeq',
  'invitedAt', 'invitedBy', 'registryVersion',
  'removedAt', 'removedBy', 'removalCommandId', 'removalReason', 'removalSeq', 'previousRole',
]);

/**
 * Removal reasons are a CLOSED VOCABULARY, not free text. A free-text reason on a staff
 * record is a private note about a named person, and this lane does not create those.
 */
...
/**
 * Is this path an enterprise membership? A `members` subcollection under any other root
 * belongs to another domain and is not evidence about enterprise staffing either way.
 */
...
/**
 * What one candidate document means. Only `foreign_ignore`, `own_active` and `own_inactive`
 * are safe to continue past; everything else is a refusal, including every case where the
 * record cannot be understood.
 */
export function evaluateMembershipCandidate(input: CandidateInput): { verdict: MembershipVerdict; detail: string } {
  const classified = classifyMembershipPath(input.path);
  if (classified.kind === 'foreign_domain') {
    // Provably not an enterprise membership: a different root, a different domain. It is
    // not evidence for OR against this grant, and treating it as either was the defect.
  ...
  // The document must agree with its own location. A record claiming an enterprise other
  // than the one it is filed under is tampered with or misplaced; either way it is not
  ...
  if (record.organizationId !== input.callerOrganizationId) {
    // Same enterprise uid, different organization: the binding moved underneath the
    // record, and re-granting across that boundary is not this callable's decision.
...
/**
 * The whole-evidence decision. `saturated` means the page came back full: the query could
 * not be shown to have returned everything, so the absence of a foreign membership in it
 * proves nothing.
 */
export function decideMembershipAdmission(input: {
  ...
  // Order matters only for the message; any one of these refuses.
...
/**
 * The identity of a removal COMMAND, length-prefixed so no two different removals can
 * produce the same fingerprint by rearranging where one field ends and the next begins.
 * An exact replay reproduces it; an altered replay does not, and must not be accepted
 * under the original command id.
 */
...
/** Command ids are caller-supplied, so they are bounded and constrained before use as a key. */
