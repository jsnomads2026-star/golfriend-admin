...
function loadEmulatorCanonical(body){const admin=require('firebase-admin'),firestore=require('firebase-admin/firestore'),apps=typeof admin.getApps==='function'?admin.getApps():admin.apps||[];if(!apps.length)admin.initializeApp({projectId:process.env.GCLOUD_PROJECT});const firestoreForVm=firestore.getFirestore;firestoreForVm.FieldValue=firestore.FieldValue;firestoreForVm.Timestamp=firestore.Timestamp;const adminForVm={...admin,apps:[{}],firestore:firestoreForVm};const source=fs.readFileSync(path.join(__dirname,'index.js'),'utf8').replace("async function activateExistingCutover(){const revision=String(process.env.K_REVISION||''),service=String(process.env.K_SERVICE||'');if(!revision||service!=='scheduledcoursecountryingestionworker')return{allowed:false,reason:'CUTOVER_WORKER_REVISION_UNAVAILABLE'};","async function activateExistingCutover(){return{allowed:true};")+'\nmodule.exports.__test={processMapping};return module.exports;',module={exports:{}};const localRequire=name=>{if(name==='firebase-functions/v2/https')return{onCall:(_options,handler)=>handler,HttpsError:class HttpsError extends Error{}};if(name==='firebase-functions/v2/scheduler')return{onSchedule:(_options,handler)=>handler};if(name==='firebase-functions/params')return{defineSecret:()=>({value:()=> 'fake-provider-key'})};if(name==='firebase-admin')return adminForVm;if(name==='node:crypto')return crypto;if(name==='./domain')return domain;if(name==='./provider-lifecycle'||name==='./calibration'||name==='./countryIngestion'||name==='./countryWorker'||name==='./countrySchedule'||name==='./countryEnrollment'||name==='./canonicalCourseMerge')return require(name);throw Error(`UNEXPECTED_REQUIRE:${name}`);};const exported=new Function('module','exports','require','console','URL','AbortSignal','process','fetch',source)(module,module.exports,localRequire,console,URL,AbortSignal,process,async()=>({ok:true,status:200,headers:{get:()=>null},text:async()=>JSON.stringify(body)}));return{db:firestore.getFirestore(),worker:exported.scheduledCourseCountryIngestionWorker,api:exported.__test};}
...
test('Firestore emulator worker writes one canonical course, protects curated data, quarantines ambiguity, and reconciles receipt totals',{skip:!process.env.FIRESTORE_EMULATOR_HOST},async()=>{
  ...
  for(const collection of ['courses','course_country_ingestion_jobs','course_country_ingestion_queue','course_country_ingestion_receipts','course_country_ingestion_cutover_receipts','golf_api_quota','golf_api_quota_reservations','golf_api_record_quarantine'])await clearEmulator(h.db,collection);
  ...
  const auditId='cutover-emulator';
  await h.db.collection('course_country_ingestion_cutover_receipts').doc(auditId).set({receiptId:auditId,immutable:true});
  await h.db.collection('platform').doc('courseCountryIngestionCutover').set({state:'active',receiptId:auditId});
  await h.db.collection('course_country_ingestion_queue').doc(jobId).set({jobId,country:'TH',ordinal:0,batch:1,state:'queued',requiresReceiptId:auditId});
  const result=await h.worker(),courseRef=h.db.collection('courses').doc('course-1'),course=(await courseRef.get()).data(),receipt=(await h.db.collection('course_country_ingestion_receipts').doc(`${jobId}-batch-1`).get()).data();
  ...
  assert.equal(receipt.counts.updated,0);
  assert.equal(receipt.counts.skipped,0);
  assert.equal(receipt.counts.quarantined,0);
  assert.equal(receipt.counts.failed,0);
  assert.deepEqual(receipt.counts,{added:1,updated:0,skipped:0,quarantined:0,failed:0,writes:receipt.courseWrites});
