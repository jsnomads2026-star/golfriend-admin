...
const expected=['armGolfApiCalibrationCanary','getGolfApiCatalogueStatus','pauseCourseCountryIngestion','reconcileGolfApiPendingSettlements','resumeCourseCountryIngestion','runGolfApiCalibrationCanary','scheduledCourseCountryIngestionWorker','scheduledGolfApiCatalogueCanary','scheduledGolfApiCatalogueCountReceipt','scheduledGolfApiCatalogueIncremental','scheduledGolfApiCatalogueRetries','searchGolfApiCatalogue','startCourseCountryIngestion'];
...
for(const required of ['enforceAppCheck:true','REFRESH_SLO','verifyActivation','golf_api_quota_reservations','pageCacheId','clubOffset','courseOffset','withPageSize','cleanupExpiredPageCaches','DETAIL_BUDGET_DEFERRED','verifyCanaryActivation','golf_api_canary_receipts','golf_api_pending_settlements','reconcileGolfApiPendingSettlements','verifyCalibrationActivation','calibration.runCalibration','golf_api_calibration_receipts','calibrationRequestsAllowed:false'])if(!source.includes(required))throw Error(`CONTROL_MISSING:${required}`);
