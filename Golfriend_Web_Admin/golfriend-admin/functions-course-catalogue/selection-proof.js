...
const expected=['getGolfApiCatalogueStatus','scheduledGolfApiCatalogueCountReceipt','scheduledGolfApiCatalogueIncremental','scheduledGolfApiCatalogueRetries','searchGolfApiCatalogue'];
...
for(const required of ['enforceAppCheck:true','REFRESH_SLO','verifyActivation','golf_api_quota_reservations','pageCacheId','clubOffset','courseOffset','withPageSize','cleanupExpiredPageCaches','DETAIL_BUDGET_DEFERRED'])if(!source.includes(required))throw Error(`CONTROL_MISSING:${required}`);
