...
async function executeProviderRequest({cost,runId,path,reserve,markInFlight,transport,digest,recordObservation,settleCompleted,persistPending,settleTransportFailure,limit=async()=>{},now=()=>new Date().toISOString()}){
  ...
  const observation={reservationId:reservation.reservationId,runId,path,cost,providerRemaining,responseDigest,observedAt:now()};
