...
export const COMMISSIONING_LOCALES=LOCALE_CODES;
...
export type TemplateApprovalProjection={
 ...
 contentDigest:string;policyVersion:string;customerApprovalRequired:boolean;status:'pending_approval'|'active'|'rejected'|'retired';version:number;
...
const text=(value:unknown,label:string,max=256)=>{if(typeof value!=='string'||!value||value.length>max)throw new Error(`${label}_MALFORMED`);return value};
...
const parseTemplate=(value:unknown):TemplateApprovalProjection=>{const x=object(value,'TEMPLATE');if(!TEMPLATE_KINDS.includes(x.kind as TemplateKind)||!COMMISSIONING_LOCALES.includes(x.locale as CommissioningLocale)||!['pending_approval','active','rejected','retired'].includes(String(x.status)))throw new Error('TEMPLATE_ENUM_MALFORMED');if(x.legalApproved!==true&&x.legalApproved!==false)throw new Error('LEGAL_APPROVAL_MALFORMED');if(x.customerApproved!==true&&x.customerApproved!==false)throw new Error('CUSTOMER_APPROVAL_MALFORMED');if(x.customerApprovalRequired!==true&&x.customerApprovalRequired!==false)throw new Error('CUSTOMER_APPROVAL_REQUIREMENT_MALFORMED');if(typeof x.contentDigest!=='string'||!/^[a-f0-9]{64}$/i.test(x.contentDigest))throw new Error('CONTENT_DIGEST_MALFORMED');return{templateApprovalId:text(x.templateApprovalId,'TEMPLATE_APPROVAL_ID'),templateId:text(x.templateId,'TEMPLATE_ID'),templateVersion:text(x.templateVersion,'TEMPLATE_VERSION'),kind:x.kind as TemplateKind,locale:x.locale as CommissioningLocale,jurisdiction:text(x.jurisdiction,'JURISDICTION'),contentDigest:x.contentDigest,policyVersion:text(x.policyVersion,'POLICY_VERSION'),customerApprovalRequired:x.customerApprovalRequired,status:x.status as TemplateApprovalProjection['status'],version:number(x.version,'VERSION'),legalApproved:x.legalApproved,customerApproved:x.customerApproved,createdAt:nullableText(x.createdAt,'CREATED_AT'),updatedAt:nullableText(x.updatedAt,'UPDATED_AT')}};
const parseSnapshot=(value:unknown):CommissioningSnapshot=>{const x=object(value,'SNAPSHOT'),boundaries=object(x.boundaries,'BOUNDARIES');if(x.schema!==ENTERPRISE_MEMBER_COMMISSIONING_SCHEMA||!['empty','current'].includes(String(x.state))||!Array.isArray(x.templates)||!Array.isArray(x.dryRuns)||!Array.isArray(x.reportValidations)||!Array.isArray(x.commissioningReceipts))throw new Error('COMMISSIONING_CONTRACT_MALFORMED');const dryRuns=x.dryRuns.map(item=>{const d=object(item,'DRY_RUN');return{dryRunId:text(d.dryRunId,'DRY_RUN_ID'),templateApprovalId:text(d.templateApprovalId,'TEMPLATE_APPROVAL_ID'),requestId:text(d.requestId,'REQUEST_ID'),status:text(d.status,'DRY_RUN_STATUS',80),providerConfigured:exactFalse(d.providerConfigured,'PROVIDER_CONFIGURED'),transmitted:exactFalse(d.transmitted,'TRANSMITTED'),createdAt:nullableText(d.createdAt,'CREATED_AT')}});const reportValidations=x.reportValidations.map(item=>{const d=object(item,'REPORT_VALIDATION');return{validationId:text(d.validationId,'VALIDATION_ID'),eventCount:number(d.eventCount,'EVENT_COUNT'),status:text(d.status,'VALIDATION_STATUS',80),commissioned:exactFalse(d.commissioned,'COMMISSIONED'),transmitted:exactFalse(d.transmitted,'TRANSMITTED'),createdAt:nullableText(d.createdAt,'CREATED_AT')}});const commissioningReceipts=x.commissioningReceipts.map(item=>{const d=object(item,'RECEIPT');if(d.immutable!==true)throw new Error('RECEIPT_IMMUTABILITY_VIOLATION');return{receiptId:text(d.receiptId,'RECEIPT_ID'),templateApprovalId:nullableText(d.templateApprovalId,'TEMPLATE_APPROVAL_ID'),subjectId:nullableText(d.subjectId,'SUBJECT_ID'),action:text(d.action,'ACTION',80),version:d.version===null?null:number(d.version,'VERSION'),createdAt:nullableText(d.createdAt,'CREATED_AT'),immutable:true as const}});return{schema:ENTERPRISE_MEMBER_COMMISSIONING_SCHEMA,state:x.state as CommissioningSnapshot['state'],templates:x.templates.map(parseTemplate),dryRuns,reportValidations,commissioningReceipts,boundaries:{legalTextStored:exactFalse(boundaries.legalTextStored,'LEGAL_TEXT'),providerConfigured:exactFalse(boundaries.providerConfigured,'PROVIDER'),jhccCommissioned:exactFalse(boundaries.jhccCommissioned,'JHCC'),notificationsSent:exactFalse(boundaries.notificationsSent,'NOTIFICATIONS'),economyWrites:exactFalse(boundaries.economyWrites,'ECONOMY')}}};
const parseCommand=(value:unknown):CommissioningCommandResult=>{const x=object(value,'COMMAND_RESULT');if(x.schema!==ENTERPRISE_MEMBER_COMMISSIONING_SCHEMA||typeof x.replayed!=='boolean')throw new Error('COMMAND_RESULT_MALFORMED');if('providerConfigured' in x&&x.providerConfigured!==false)throw new Error('PROVIDER_CONFIGURED_BOUNDARY_VIOLATION');if('transmitted' in x&&x.transmitted!==false)throw new Error('TRANSMITTED_BOUNDARY_VIOLATION');return{templateApprovalId:typeof x.templateApprovalId==='string'?text(x.templateApprovalId,'TEMPLATE_APPROVAL_ID'):undefined,dryRunId:typeof x.dryRunId==='string'?text(x.dryRunId,'DRY_RUN_ID'):undefined,validationId:typeof x.validationId==='string'?text(x.validationId,'VALIDATION_ID'):undefined,receiptId:text(x.receiptId,'RECEIPT_ID'),status:text(x.status,'STATUS',80),version:x.version===undefined?undefined:number(x.version,'VERSION'),replayed:x.replayed,providerConfigured:'providerConfigured' in x?false:undefined,transmitted:'transmitted' in x?false:undefined}};
