...
import{validateJhccPayload}from'./acquisitionReportingModel.mjs';
...
export const ADAPTER_ERROR_CODES=Object.freeze(['ADAPTER_UNAVAILABLE','AUTHORIZATION_REQUIRED','VALIDATION_FAILED','PRIVACY_SCREEN_FAILED','IDEMPOTENCY_CONFLICT','NOT_ELIGIBLE']);
...
const adapterString=(value)=>typeof value==='string'&&value.trim().length>0?value.trim():null;
...
export async function loadAcquisitionSource(adapters,{limit=100}={}){
  ...
  const call=await callAdapter(id,()=>adapter.load({limit}));
  if(!call.ok)return fail(id,'ADAPTER_UNAVAILABLE',`The acquisition source failed: ${call.message}`);
  if(!Array.isArray(call.value))return fail(id,'VALIDATION_FAILED','The source did not return a row array.');
  return ok(id,{source:adapterString(adapter.label)||'unlabelled source',prospects:call.value.slice(0,limit).map(outboundProspect)});
}
...
export async function deliverOutreach(adapters,{draft,recipientSelected=false,humanApproved=false}={}){
  ...
  if(!call.ok)return fail(id,'ADAPTER_UNAVAILABLE',`The outreach adapter failed: ${call.message}`);
...
export async function submitConversionHandoff(adapters,{prospect,idempotencyKey}={}){
  ...
  if(!call.ok)return fail(id,'ADAPTER_UNAVAILABLE',`The conversion adapter failed: ${call.message}`);
 ...
 */
export async function transmitJhccAcquisition(adapters,{payload,authorization,confirmed=false,idempotencyKey}={}){
  ...
  if(!call.ok)return fail(id,'ADAPTER_UNAVAILABLE',`The JHCC transmitter failed: ${call.message}`);
