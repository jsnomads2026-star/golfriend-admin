...
export default function V2CourseCoverageByCountry({ service }: { service: CourseOperationsService }) {
  ...
  return <section className="course-catalogue" aria-labelledby="course-coverage-by-country-title">
    ...
    {globalPreviewState==='ready'&&globalPreview&&<section className="course-state" role="status" aria-live="polite"><h4>Global Preview summary</h4><p>Receipt: {String(globalPreview.receiptId||'unavailable')}. Schema: {String(globalPreview.schema||'unavailable')}. Provider calls: {String(globalPreview.providerCalls)}. Course writes: {String(globalPreview.courseWrites)}.</p><p>Automatic refresh currently {globalPreview.enabled===true?'enabled':'disabled'}; this preview did not change it. Queue entries: {Array.isArray(globalPreview.queue)?globalPreview.queue.length:0}. First country: {String((globalPreview.firstCountry as Record<string,unknown>|null)?.country||'none')}.</p></section>}
