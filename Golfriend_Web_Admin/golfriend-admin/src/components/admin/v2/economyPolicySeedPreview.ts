...
const action = (actionId: string, direction: Direction, amountTee: number) => ({ actionId, direction, amountTee, lifecycle: direction === 'earn' ? { reserve: 'prohibited', settle: 'not_applicable_issuance_is_immediate', release: 'prohibited' } : direction === 'spend' ? { reserve: 'prohibited', settle: 'required_immediate_debit', release: 'prohibited' } : { reserve: 'required_hold_exact_amount', settle: 'required_consume_exact_reserved_amount', release: 'required_return_exact_reserved_amount_when_unsettled' } } as const);

// Immutable client rendering copy of the committed Director seed preview. It is
// used only after the existing server-owned Director gate has authorized Admin.
export const DIRECTOR_POLICY_SEED_PREVIEW = Object.freeze({
  approvalState: 'director_approval_required' as const,
  ...
  teePacks: [{ id: 'tees_100', teeCount: 100, currency: 'USD', priceUsd: 10 }, { id: 'tees_250', teeCount: 250, currency: 'USD', priceUsd: 25 }, { id: 'tees_500', teeCount: 500, currency: 'USD', priceUsd: 50 }],
  lotRules: { promotionalTtlDays: 180, consumptionOrder: ['promotional_expiring_first', 'purchased_and_member_earned_by_issued_at_then_lot_id'] },
  actions: [
