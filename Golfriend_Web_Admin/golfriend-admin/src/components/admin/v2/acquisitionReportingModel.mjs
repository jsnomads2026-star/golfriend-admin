...
const demandRollup=(rows)=>{const attribution=rollupAttribution(rows.map((r)=>r.demand.attribution));const metric=(key,requireAuthoritative)=>({...discloseMetric(sumOrNull(rows.map((r)=>r.demand[key])),attribution,{requireAuthoritative})});return{attribution,searchInterest:metric('searchInterest',false),savedCourse:metric('savedCourse',false),bookingInterest:metric('bookingInterest',false),confirmedBookings:metric('confirmedBookings',true),playedRounds:metric('playedRounds',true)};};
...
export function acquisitionReportToText(report){return[`Schema: ${report.schema} (version ${report.version})`,`Generated: ${report.generatedAt}`,`Period: ${report.period.start} to ${report.period.end}`,`Prospects: ${report.analytics.totals.prospects} across ${report.analytics.totals.countries} country/countries`,`Commission-effective: ${report.analytics.totals.commissionEffective} · opportunity evidence only: ${report.analytics.totals.opportunityEvidenceOnly}`,'',...report.analytics.countries.map((c)=>`${c.country}: ${c.prospects} prospect(s) | attribution ${c.demand.attribution} | booking interest ${c.demand.bookingInterest.disclosed?c.demand.bookingInterest.value:`withheld (${c.demand.bookingInterest.reason})`}`),'',`JHCC delivery: ${report.delivery.status} (${report.delivery.reason})`,report.delivery.notice,...report.limitations].join('\n');}
export function acquisitionReportToCsv(report){return[['country','prospects','commission_effective','attribution','booking_interest','booking_interest_state','confirmed_bookings_state'],...report.analytics.countries.map((c)=>[c.country,c.prospects,c.signed,c.demand.attribution,c.demand.bookingInterest.disclosed?c.demand.bookingInterest.value:'',c.demand.bookingInterest.reason,c.demand.confirmedBookings.reason])].map((row)=>row.map(esc).join(',')).join('\n');}
