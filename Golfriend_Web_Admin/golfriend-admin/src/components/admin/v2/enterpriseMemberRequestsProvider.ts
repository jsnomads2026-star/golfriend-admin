...
export type MemberRequestDetail = {request:MemberRequestProjection; evidenceDigest:string; history:ReceiptProjection[]; delivery?:DeliveryProjection|null; consent?:{status:string;version:number;receiptIds:string[]}; lifecycle?:{status:string;version:number}; csv?:{previewId:string;rowDigest:string;rows:Array<{row:number;status?:string;memberReference:string;conflict?:string}>}};
...
export const firebaseEnterpriseMemberAdminProvider:EnterpriseMemberAdminProvider={
  ...
  detail:async(requestId)=>{const raw:any=await call('getEnterpriseMemberRequestAdminV1',{requestId});return{...raw,request:request(raw.request),history:(raw.history||[]).map((item:any)=>({receiptId:item.receiptId,action:item.action,status:item.status||'recorded',actorType:item.reviewerRole?'admin':'system',at:item.occurredAt,reference:item.reference})),delivery:Array.isArray(raw.delivery)&&raw.delivery.length?delivery(raw.delivery[0]):null,consent:raw.lifecycle?{status:raw.lifecycle.consentState,version:Number(raw.lifecycle.consentVersion||0),receiptIds:[]}:undefined,lifecycle:{status:raw.request?.status||'unavailable',version:Number(raw.request?.version||0)}}},
