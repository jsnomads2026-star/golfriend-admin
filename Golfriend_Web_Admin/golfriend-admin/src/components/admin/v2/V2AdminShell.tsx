...
import { ADMIN_AREAS, ADMIN_LOCALES, type AdminArea, type AdminLocale } from './adminNavigation';
...
export default function V2AdminShell({ activeArea, onAreaChange, onSignOut, children }: { activeArea: AdminArea; onAreaChange: (area: AdminArea) => void; onSignOut: () => void; children: ReactNode }) {
  ...
  const copy = ADMIN_PRESENTATION[locale];
  ...
  return <AdminLocaleContext.Provider value={locale}><div className="v2-admin-shell">
    <aside className={`v2-admin-sidebar ${menuOpen ? 'is-open' : ''}`} aria-label={copy.primaryNavigation}>
      ...
      <nav className="v2-admin-nav">{ADMIN_AREAS.map((area, index) => <button key={area} type="button" aria-current={activeArea === area ? 'page' : undefined} onClick={() => onAreaChange(area)}><span>{String(index + 1).padStart(2, '0')}</span>{copy[area]}</button>)}</nav>
