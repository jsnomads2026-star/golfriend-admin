...
/**
 * How the SERVER predicate will treat this record. Rendering only — nothing here grants
 * anything, and the server re-derives authority on every callable. It exists so this
 * roster tells the truth: now that the role registry is closed, a record carrying an
 * out-of-registry role is denied by the server, and it must not appear here as ordinary
 * staff on the one screen an operator uses to diagnose exactly that.
 */
function classifyStaffRecord(record: any): { effective: 'active' | 'denied'; reason: string; detail: string } {
  if (!record || typeof record !== 'object') return { effective: 'denied', reason: 'malformed', detail: 'Malformed record' };
  const status = normalizeStaffStatus(record.status);
  if (status === null) return { effective: 'denied', reason: 'missing_status', detail: 'No status — access denied' };
  ...
  if (!isCanonicalAdminRole(record.role)) {
    // Legacy, mis-spelled and foreign-vocabulary roles all land here. The repair is a
    // data correction, not a suspension, so the roster says which it is.
    return {
      effective: 'denied',
      reason: 'unknown_role',
...
export default function HRManagement() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // out-of-registry role rendered as ordinary staff.
            ...
            <tbody>
              {staff.map((emp) => (
                <tr key={emp.id} style={{ borderBottom: '1px solid #333', opacity: emp.status === 'Suspended' ? 0.5 : 1 }}>
                  ...
                  <td style={{ padding: '12px 0', color: emp.authority?.effective === 'active' ? '#D4AF37' : '#F44336' }}>{emp.role || '—'}{emp.authority?.effective !== 'active' && (<div style={{ fontSize: '11px', color: '#F44336' }}>{emp.authority?.detail}</div>)}</td>
