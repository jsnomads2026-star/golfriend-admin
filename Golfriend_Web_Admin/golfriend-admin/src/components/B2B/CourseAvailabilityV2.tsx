...
const call = async (name: string, data: Record<string, unknown> = {}) =>
  (await httpsCallable(functions, name)(data)).data as { slots: Array<Record<string, unknown>>; readOnly?: boolean };
...
const COPY: Record<string, typeof EN> = {
  en: EN,
  th: { ...EN, title: 'เวลาว่างที่สนามกอล์ฟเป็นผู้ให้บริการ', loading: 'กำลังโหลดเวลาว่าง…', empty: 'ยังไม่มีช่วงเวลา', error: 'ระบบเวลาว่างไม่พร้อมใช้งาน ไม่มีการเปลี่ยนแปลง', boundary: 'Golfriend ประสานการจองกับบุคคลที่สามเท่านั้น ไม่รับชำระ ค่าธรรมเนียม หรือการชำระบัญชี' },
  ko: { ...EN, title: '코스 제공 예약 가능 시간' }, ja: { ...EN, title: 'コース提供の空き時間' }, zh: { ...EN, title: '球场提供的可用时段' },
  es: { ...EN, title: 'Disponibilidad proporcionada por el campo' }, fr: { ...EN, title: 'Disponibilités fournies par le parcours' }, de: { ...EN, title: 'Vom Platz bereitgestellte Verfügbarkeit' },
};
...
export default function CourseAvailabilityV2({ admin = false }: { admin?: boolean }) {
  ...
  const [data, setData] = useState<{ slots: Array<Record<string, unknown>>; readOnly?: boolean } | null>(null);
  const [state, setState] = useState<'loading' | 'ready' | 'error'>('loading');
  const [busy, setBusy] = useState(false);
  const [courseId, setCourseId] = useState(''); const [date, setDate] = useState(''); const [time, setTime] = useState('');
  const [timeZone, setTimeZone] = useState('Asia/Bangkok'); const [capacity, setCapacity] = useState(4); const [notice, setNotice] = useState('');
  ...
  const run = async (fn: () => Promise<unknown>) => { setBusy(true); try { const result = await fn() as { receiptId?: string }; setNotice(`OK · ${result.receiptId}`); await load(); } catch { setNotice(copy.error); } finally { setBusy(false); } };
