      ...
      <div style={{ display: 'flex', gap: '20px', marginBottom: '30px' }}>
        <div style={{ flex: 1, backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px', padding: '20px' }}>
          ...
          <div style={{ color: '#555', fontSize: '11px', marginTop: '5px' }}>
            Allowed in {isMasterHost ? 'Master Host ($599)' : 'Basic ($299)'} Tier
          </div>
        </div>
        <div style={{ flex: 1, backgroundColor: '#111', border: '1px solid #333', borderRadius: '8px', padding: '20px' }}>
...
// InboxCRM dummy component removed for 299 Tier
...
export default function SmallBusinessDashboard({ partnerData }: PartnerDashboardProps) {
  ...
  // 🔥 FIREBASE AUTH FIX: Force lowercase to match strict database rules
  const userEmail = (getAuth().currentUser?.email || partnerData?.email || "").toLowerCase();

  useEffect(() => {
    if (!userEmail) return;
    
    const docRef = doc(db, 'b2b_partners', userEmail);
    const unsubscribe = onSnapshot(docRef, (snapshot) => {
      if (snapshot.exists()) {
        const data = snapshot.data();
        if (data.sponsorCredits !== undefined) setDbCredits(data.sponsorCredits);
      } else {
        console.warn(`Database document for ${userEmail} not found. Check case sensitivity.`);
      }
    });
    return () => unsubscribe();
  }, [userEmail]);

  // 🔥 TRUE TIER EVALUATION ENFORCED FOR 299
  const isMasterHost = false; 

  // 🔥 CREDIT MATH FIX: Basic Tier enforces 3000 limit unless explicitly overridden
  const displayCredits = dbCredits !== null ? dbCredits : 3000;
    ...
    <div style={styles.masterContainer}>
      <div style={styles.sidebar}>
        <div style={styles.headerBlock}>
          <GolfText style={styles.logo}>GOLFRIEND B2B</GolfText>
          <GolfText style={styles.tierBadge}>🤝 BASIC OPERATOR</GolfText>
          <GolfText style={styles.creditBalance}>{displayCredits.toLocaleString()} Credits</GolfText>
        </div>
...
const styles = {
  masterContainer: { display: 'flex', minHeight: '100vh', backgroundColor: '#121212', color: 'white', fontFamily: 'sans-serif' },
  sidebar: { width: '260px', backgroundColor: '#0a0a0a', padding: '24px 16px', borderRight: '1px solid #D4AF37', display: 'flex', flexDirection: 'column' as const },
  ...
  navBtn: { 
    ...
    textAlign: 'left' as const, cursor: 'pointer', borderRadius: '24px', fontSize: '14px', fontWeight: 'bold' as const, transition: 'all 0.2s ease'
  },
