...
export interface LocationProjection {locationId:string;name?:string;label:string;address:string;city:string;countryCode:string;country:string;serviceArea:string;operatingHours?:Array<{day:number;opens?:string;closes?:string;closed:boolean}>;hours:Array<{day:number;opens?:string;closes?:string;closed:boolean}>;}
export interface EvidenceProjection {evidenceId:string;kind:string;objectReference:string;contentDigest:string}
...
export interface PortalProjection {schema:typeof SMALL_BUSINESS_SCHEMA;state:'current'|'empty'|'unavailable'|'conflict';business?:{businessId:string;version:number;legalName:string;displayName:string;registrationReference:string;category:BusinessCategory;status:BusinessStatus;serviceDescription:string;serviceArea:string;supportedLocales:SmallBusinessLocale[]};locations:LocationProjection[];evidenceReferences?:EvidenceProjection[];plans:PlanProjection[];promotions:Array<{promotionId:string;version:number;status:'draft'|'pending_review'|'approved'|'active'|'expired'|'rejected';locale:SmallBusinessLocale;jurisdiction:string;effectiveAt:string;expiresAt:string;contentDigest:string;disclosure:'sponsored'}>;receipts:ReceiptProjection[];supportReference?:string;}
